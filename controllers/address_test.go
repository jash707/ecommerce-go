...
func TestEditWorkAddress(t *testing.T) {
	...
	// Set up the route for the EditHomeAddress handler
	router.PUT("/editworkaddress", EditWorkAddress())
	...
	// Create a new HTTP request with PUT method
	req, err := http.NewRequest(http.MethodPut, "/editworkaddress?userID="+mockUser.User_ID, bytes.NewBuffer(mockEditAddressJSON))
	if err != nil {
		t.Fatalf("could not create request: %v", err)
	}
	...
	// Verify the response body matches the expected plain JSON string
	expectedResponse := "\"Successfully updated the work address\""
	assert.Equal(t, expectedResponse, recorder.Body.String())
}
